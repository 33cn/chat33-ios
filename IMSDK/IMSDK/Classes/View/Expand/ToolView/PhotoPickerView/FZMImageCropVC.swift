...
class FZMImageCropVC: FZMBaseViewController {
    
    var confirmBlock : ImageBlock?
    ...
    private let overlayView: UIView = {
        let view = UIImageView(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: ScreenHeight))
    ...
    private let cropView: UIView = {
        let view = UIView(frame: CGRect(x: 0 , y: (ScreenHeight - ScreenWidth - StatusNavigationBarHeight) / 2, width: ScreenWidth, height: ScreenWidth))
    ...
    private func initViewsFrame(){
        ...
        // 判断选中图片OldHeight
        if photoOldHeight > ScreenHeight {
            // 大于屏幕尺寸,则正常展示
            self.backImageView.frame = CGRect(x: 0, y: 0, width: cropWidth, height: photoOldHeight)
        }else{
            // 小于屏幕尺寸,将其置于中心展示
            self.backImageView.frame = CGRect(x: 0, y: (ScreenHeight - photoOldHeight) / 2 , width: cropWidth, height: photoOldHeight)
        }
        /// 获取照片展示oldFrame
    ...
    private func drawBottomButtonView(){
        self.navigationItem.rightBarButtonItem = UIBarButtonItem(title: "确定", style: .plain, target: self, action: #selector(onClickConfirmButton))
    }
    ...
    @objc private func onClickConfirmButton(){
        let image = self.getSubImage()
        self.confirmBlock?(image)
        self.popBack()
    }
    /// 获取截取图片
