...
extension UIButton{
    ...
    class func getNormalBtn(with title: String , backgroundColor: UIColor = FZM_TintColor) -> UIButton {
        let btn = UIButton(type: .custom)
        btn.makeOriginalShdowShow()
        btn.layer.cornerRadius = 20
        
        btn.layer.backgroundColor = backgroundColor.cgColor
        btn.setAttributedTitle(NSAttributedString(string: title, attributes: [.foregroundColor: backgroundColor == FZM_TintColor ? UIColor.white : FZM_GrayWordColor,.font:UIFont.regularFont(16)]), for: .normal)
        return btn
    }
}
