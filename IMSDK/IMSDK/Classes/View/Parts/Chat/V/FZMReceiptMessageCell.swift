...
class FZMReceiptMessageCell: FZMBaseMessageCell {
    ...
    lazy var contentImageView : UIImageView = {
        ...
        v.backgroundColor = FZM_ReceiptBgColor
        return v
    }()
    ...
    lazy var transferIconImageView : UIImageView = {
        ...
        v.image = GetBundleImage("input_receipt")
        return v
    }()
    ...
    override func configure(with data: FZMMessageBaseVM) {
        ...
        guard let data = data as? FZMReceiptMessageVM  else { return }
        self.moneyLab.text = data.money
        self.inforLab.text = data.infor
        if data.recordId.isEmpty {
            self.contentImageView.backgroundColor = FZM_ReceiptBgColor
            self.transferIconImageView.image = GetBundleImage("input_receipt")
        } else {
            self.contentImageView.backgroundColor = FZM_ReceiptDoneBgColor
            self.transferIconImageView.image = GetBundleImage("input_receipt_done")
        }
    }
    
    func contentImageViewTap(msgId: String) {
        guard let vm = self.vm as? FZMReceiptMessageVM else { return }
        self.actionDelegate?.openReceipt(msgId: msgId)
    }
...
class FZMMineReceiptMessageCell: FZMReceiptMessageCell {
    override func initView() {
        super.initView()
